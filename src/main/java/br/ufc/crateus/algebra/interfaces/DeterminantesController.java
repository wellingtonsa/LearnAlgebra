...
public class DeterminantesController extends Application implements Initializable {
	...
	public void start(Stage primaryStage) throws Exception {
		DeterminantesController.myStage = primaryStage;
		Pane pane = (Pane) FXMLLoader.load(FXMLResources.DETERMINANTES);
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		iCalculator.setOnMousePressed(new EventHandler<Event>() {
			...
			public void handle(Event event) {
				try {
					new CalculatorController().start(new Stage());
				} catch (Exception e) {
					e.printStackTrace();
				}
